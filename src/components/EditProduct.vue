...
export default {
  setup() {
    // Defining product as a reactive reference
    const product = ref({ id: null, name: '', description: '', price: '' });
    const errors = ref({});
    
    // Using the useRoute method to access the route parameters
    ...
    // Fetch the product from the API when the component is created
    onMounted(async () => {
      try {
    ...
    // Returning the data and methods to use them in the template
